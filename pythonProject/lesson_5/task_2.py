...
def repeats(our_str):
    letters_dict = {} # обозначили новый словарь
    new_str = '' # обозначили новую строку

    for i in our_str:
        if i in letters_dict: # Если элемент списка входит в словарь, то прибавляем +1, иначе добавляем в словарь
            letters_dict[i] = letters_dict[i] + 1
            new_str = new_str + i + '_' + str(letters_dict[i])
        else:
            letters_dict[i] = 1
            new_str = new_str + i + '_' + str(letters_dict[i])
