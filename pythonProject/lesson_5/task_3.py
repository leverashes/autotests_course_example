...
def everything_for_your_cat(cats_data):
    ...
    cats = []
    new_cats_data = []
    # Перебираем тот же список, но делаем список из списков + объеденим ФИО
    ...
    # В словарь boss_dict запихиваем кошек cats
    for i in range(len(new_cats_data)):
        if boss_dict[new_cats_data[i][2]] is None:
            boss_dict[new_cats_data[i][2]] = (cats[i])
        else:
            a = boss_dict.get(new_cats_data[i][2])
            boss_dict[new_cats_data[i][2]] = (a +'; ' + cats[i])

    # Выводим так как надо
    our_str = ''.join([str(key) + ': ' + str(value) + '\n' for key, value in boss_dict.items()])
    print(our_str)
